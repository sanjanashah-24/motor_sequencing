...
expName = 'motor_sequencing'  # from the Builder filename that created this script
expInfo = {
...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='C:\\Users\\Sanjana\\Desktop\\PSYCHOLOGY LAB\\data\\motor_sequencing_lastrun.py',
...
for thisTrial in trials:
    ...
    probe.setPos((pos, -200))
    key_resp.keys = []
    key_resp.rt = []
    _key_resp_allKeys = []
    # keep track of which components have finished
    trialComponents = [fixation, fixation_2, line1, line2, line3, line4, probe, key_resp]
    ...
    while continueRoutine:
        ...
        if fixation.status == STARTED:
            ...
            if tThisFlipGlobal > fixation.tStartRefresh + 0.500-frameTolerance:
        ...
        if fixation_2.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > fixation_2.tStartRefresh + 0.700-frameTolerance:
                # keep track of stop time/frame for later
...
# set up handler to look after randomisation of conditions etc
trials_2 = data.TrialHandler(nReps=40.0, method='fullRandom', 
    extraInfo=expInfo, originPath=-1,
    trialList=data.importConditions('sequence_cond.csv'),
...
# completed 40.0 repeats of 'trials_2'
